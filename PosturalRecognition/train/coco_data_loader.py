...
from PosturalRecognition.train.entity import JointType, params
...
class CocoDataLoader(DatasetMixin):
    ...
    def gen_constant_paf(self, imshape, joint_from, joint_to, paf_width):
        ...
        constant_paf = np.stack((paf_flag, paf_flag)) * np.broadcast_to(unit_vector, imshape[:-1] + (2,)).transpose(2,                                                                                                    1)
        return constant_paf
    ...
    def gen_gaussian_paf(self, imshape, joint_from, joint_to, paf_sigma):
        ...
        gaussian_paf = np.stack((horizontal_paf_flag * gauss, horizontal_paf_flag * gauss)) * np.broadcast_to(
            unit_vector, imshape[:-1] + (2,)).transpose(2, 0, 1)
        return gaussian_paf
    ...
    def random_crop_img(self, orig_img, mask_img, joints, valid_joints, joint_bboxes, min_crop_size):
        ...
        while True:
            ...
            valid_iob_list, iob_list = self.validate_crop_area([[crop_left, crop_top], [crop_right, crop_bottom]],
                                                               joint_bboxes, params['crop_iob_thresh'])
    ...
    def augment_data(self, orig_img, mask_img, joints, valid_joints, joint_bboxes, min_crop_size):
        ...
        augmented_img, augmented_mask, joints, valid_joints = self.random_crop_img(augmented_img, augmented_mask,
                                                                                   joints, valid_joints, joint_bboxes,
                                                                                   min_crop_size)
        ...
        if np.random.randint(2):
            augmented_img, augmented_mask, joints, valid_joints = self.flip_img(augmented_img, augmented_mask, joints,
                                                                                valid_joints)
        ...
        for limb in params['limbs_point']:
            ...
            for person_index, person_joints in enumerate(joints):
                if valid_joints[person_index][limb[0]] and valid_joints[person_index][limb[1]]:
                    limb_paf = self.gen_constant_paf(img.shape, np.array(person_joints[limb[0]]),
                                                     np.array(person_joints[limb[1]]), paf_sigma)
    ...
    def compute_pafs_gaussian(self, img, joints, valid_joints, paf_sigma):
        ...
        for limb in params['limbs_point']:
            ...
            for person_index, person_joints in enumerate(joints):
                if valid_joints[person_index][limb[0]] and valid_joints[person_index][limb[1]]:
                    limb_paf = self.gen_gaussian_paf(img.shape, np.array(person_joints[limb[0]]),
                                                     np.array(person_joints[limb[1]]), paf_sigma)
    ...
    def parse_coco_annotation(self, img, annotations):
        ...
        for annotation in annotations:
            ...
            person_joint_bbox = np.array(
                [[[np.iinfo(np.int32).max, np.iinfo(np.int32).max], [np.iinfo(np.int32).min, np.iinfo(np.int32).min]]],
                np.int32)
            ...
            for i, joint_index in enumerate(params['coco_joint_indices']):
                ...
                if valid_joint:
                    ...
                    person_joint_bbox[0][0][0] = np.minimum(person_joint_bbox[0][0][0],
                                                            person_joints[0][joint_index][0])  # left
                    person_joint_bbox[0][0][1] = np.minimum(person_joint_bbox[0][0][1],
                                                            person_joints[0][joint_index][1])  # top
                    person_joint_bbox[0][1][0] = np.maximum(person_joint_bbox[0][1][0],
                                                            person_joints[0][joint_index][0])  # right
                    person_joint_bbox[0][1][1] = np.maximum(person_joint_bbox[0][1][1],
                                                            person_joints[0][joint_index][1])  # bottom

            # 计算脖子的位置
            if bool(person_valid_joints[0][JointType.LeftShoulder]) and bool(
                    person_valid_joints[0][JointType.RightShoulder]):
                person_valid_joints[0][JointType.Neck] = True
                person_joints[0][JointType.Neck][0] = int(
                    (person_joints[0][JointType.LeftShoulder][0] + person_joints[0][JointType.RightShoulder][0]) / 2)
                person_joints[0][JointType.Neck][1] = int(
                    (person_joints[0][JointType.LeftShoulder][1] + person_joints[0][JointType.RightShoulder][1]) / 2)
    ...
    def get_sample(self, img, annotations, ignore_mask):
        ...
        if self.mode != 'eval':
            img, ignore_mask, joints, valid_joints = self.augment_data(img, ignore_mask, joints, valid_joints,
                                                                       joint_bboxes, crop_size)
        resized_img, resized_mask, resized_joints = self.resize_data(img, ignore_mask, joints,
                                                                     resize_shape=(input_size, input_size))
        downscaled_img, downscaled_mask, downscaled_joints = self.resize_data(img, ignore_mask, joints, resize_shape=(
    ...
    # 从一个img索引ID提取COCO标注数据，不满足条件返回None
    def get_img_annotation(self, ind=None, img_id=None):
        annotations = None

        if ind is not None:
            img_id = self.imgIds[ind]
        # print("ind=" + (str(ind) if ind is not None else "None") + ", img_id=" + (str(img_id) if ind is not None else "None"))
        ...
        try:
            ...
            print("ind=" + (str(ind) if ind is not None else "None") + ", img_id=" + (
                str(img_id) if ind is not None else "None"))
            print("img")
            print(e)
        try:
...
if __name__ == '__main__':
    ...
    for i in range(len(data_loader)):
        ...
        if annotations is not None:
            ...
            augmented_img, augmented_mask, joints, valid_joints = data_loader.augment_data(img, ignore_mask, joints,
                                                                                           valid_joints, joint_bboxes,
                                                                                           min_crop_size=480)

            resized_img, resized_mask, resized_joints = data_loader.resize_data(augmented_img, augmented_mask, joints,
                                                                                resize_shape=(368, 368))
            downscaled_img, downscaled_mask, downscaled_joints = data_loader.resize_data(augmented_img, augmented_mask,
                                                                                         joints, resize_shape=(46, 46))

            # 计算pafs和heatmaps
            downscaled_pafs = data_loader.compute_pafs(downscaled_img, downscaled_joints, valid_joints,
                                                       params['paf_sigma'])
            downscaled_heatmaps = data_loader.compute_heatmaps(downscaled_img, downscaled_joints, valid_joints,
                                                               params['heatmap_sigma'])
