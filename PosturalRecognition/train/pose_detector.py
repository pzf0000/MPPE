...
from PosturalRecognition.train.entity import params, JointType
    ...
    def create_gaussian_kernel(self, sigma=1, ksize=5):
        ...
        for y in range(ksize):
            ...
            for x in range(ksize):
                ...
                kernel[0][0][y][x] = 1 / (sigma ** 2 * 2 * np.pi) * np.exp(
                    -(distance_x ** 2 + distance_y ** 2) / (2 * sigma ** 2))
        return kernel

    def compute_optimal_size(self, orig_img, img_size):
        """Adjust the size of image height and width of a multiple of 8 """
        orig_img_h, orig_img_w, _ = orig_img.shape
        aspect = float(orig_img_h) / float(orig_img_w)
    ...
    def compute_peaks_from_heatmaps(self, heatmaps):
        ...
        if xp == np:
            ...
            for i, heatmap in enumerate(heatmaps):
                ...
                peaks_binary = xp.logical_and.reduce((heatmap >= map_left, heatmap >= map_right, heatmap >= map_top,
                                                      heatmap >= map_bottom, heatmap > params['heatmap_peak_thresh']))
            ...
            heatmaps = F.convolution_2d(heatmaps[:, None], self.gaussian_kernel, stride=1,
                                        pad=int(params['ksize'] / 2)).data.squeeze()
    ...
    def compute_candidate_connections_greedy(self, paf, cand_a, cand_b, img_len, params):
        ...
        for index_a, joint_a in enumerate(cand_a):
            for index_b, joint_b in enumerate(cand_b):
                ...
                integ_value_with_dist_prior = integ_value + min(params['length_penalty_ratio'] * img_len / vec_len - 1,
                                                                0)
                ...
                if n_valid_points > params['n_integ_points_thresh'] and integ_value_with_dist_prior > 0:
                    candidate_connections.append([index_a, index_b, integ_value_with_dist_prior,
                                                  integ_value_with_dist_prior + joint_a[2] + joint_b[2]])
                ...
                for c in candidate_connections:
                    ...
                    if index_a not in connections[:, 3] and index_b not in connections[:, 4]:
                        connections = np.vstack(
                            [connections, [cand_a[index_a][3], cand_b[index_b][3], score, index_a, index_b]])
        ...
        for connection_category_index in range(len(params['limbs_point'])):
            ...
            for connection_index, _ in enumerate(all_connections[connection_category_index]):
                ...
                for subset_index, subset in enumerate(subsets):
                    if subset[joint_category_a_index] == joint_a_indices[connection_index] or subset[
                        joint_category_b_index] == joint_b_indices[connection_index]:
                        joint_found_subset_index[joint_found_cnt] = subset_index
                        joint_found_cnt += 1

                if joint_found_cnt == 1:
                    found_subset = subsets[joint_found_subset_index[0]]
                    # 肩->耳朵的connection组合，必须起点一致。肩->耳朵的过程终点一致
                    if found_subset[joint_category_b_index] != joint_b_indices[connection_index]:
                        ...
                        # joint b的score和connection积分值相加
                        found_subset[-2] += candidate_peaks[joint_b_indices[connection_index].astype(int), 3] + \
                                            all_connections[connection_category_index][connection_index][2]

                elif joint_found_cnt == 2:  # subset 1 joint 1，2 subset joint 2的情况（有可能是肩膀->耳朵的connection组合）
                ...
                elif joint_found_cnt == 0 and connection_category_index < 17:  # 肩膀耳朵的connection在group对象外
                    row = -1 * np.ones(20)
                    row[joint_category_a_index] = joint_a_indices[connection_index]
                    row[joint_category_b_index] = joint_b_indices[connection_index]
                    row[-1] = 2
                    row[-2] = sum(candidate_peaks[
                                      all_connections[connection_category_index][connection_index, :2].astype(
                                          int), 3]) + all_connections[connection_category_index][connection_index][2]
...
if __name__ == '__main__':
    ...
    # load model
    pose_detector = PoseDetector(args.arch, args.weights)
